...
public class TabelaBotao {
	...
	public boolean doisBotoes = true;
	Botao liga_desliga2;
	...
	public TabelaBotao(String textoDeCima, int x, int y, int minLargura, int corDeCima, int corDoCorpo, int textoDeCima_cor, int alpha, boolean ativado, boolean amostra, BotaoEventListener BEL){
	...
	public TabelaBotao(String textoDeCima, int x, int y, int minLargura, int corDeCima, int corDoCorpo, int textoDeCima_cor, int alpha, boolean ativado, boolean amostra){
		this.textoDeCima = textoDeCima;
		this.x = x;
		this.y = y;
		this.minLargura = minLargura;
	...
	public void desenhar(int mouseX, int mouseY, int telaLargura, int telaAltura, Collection<BotaoInformacoes> btns_info){
		...
		for(BotaoInformacoes btn_info : btns_info){
			if(fr.getStringWidth(btn_info.texto)+8 > larguraMax){
				larguraMax = fr.getStringWidth(btn_info.texto)+8;
			}
		...
		if(!amostra)
			return;
		Desenhador.desenharRetanguloTransparente(x, y, larguraMax, 2, corDoCorpo, alpha-(int)(5*2.55));
		y += 2;
		botoes.clear();
		for(BotaoInformacoes btn_info : btns_info){
			int btnX = x+2;
			...
			Desenhador.desenharRetanguloTransparente(x, y, larguraMax, fr.FONT_HEIGHT+4+2, corDoCorpo, alpha-(int)(5*2.55));
			Botao botao = new Botao(btnX, btnY, btnLargura, btnAltura, btn_info.ID, btn_info.texto, true, false);
			botao.margem = 2;
			botao.defineApenasAlpha((int)(20*2.55));
			botao.defineApenasAlphaHover((int)(40*2.55));
