...
public class Lista {
	...
	public boolean ativado = false;//temporario e nao serve pra nada
	public boolean amostra = true;
	
	
	FontRenderer fr = Minecraft.getMinecraft().fontRenderer;
	public Lista(String textoDeCima, int x, int y, int minLargura, int corDeCima, int corDoCorpo, int textoDeCima_cor, int alpha, boolean ativado, boolean amostra, BotaoEventListener BEL){
	...
	public Lista(String textoDeCima, int x, int y, int minLargura, int corDeCima, int corDoCorpo, int textoDeCima_cor, int alpha, boolean ativado, boolean amostra){
		this.textoDeCima = textoDeCima;
		this.x = x;
		this.y = y;
		this.minLargura = minLargura;
	...
	public void desenhar(int mouseX, int mouseY, int telaLargura, int telaAltura, Collection<TextoInformacoes> lista){
		...
		if(amostra)
			AlturaMax += 2;
		for(TextoInformacoes texto : lista){
			if(fr.getStringWidth(texto.texto)+4 > larguraMax){
				larguraMax = fr.getStringWidth(texto.texto)+4;
			}
		...
		for(TextoInformacoes texto : lista){
			int textoX = x+2;
			int textoY = y;
			int textoLargura = larguraMax-4;
			int textoAltura = fr.FONT_HEIGHT+4;
			Desenhador.desenharRetanguloTransparente(x, y, larguraMax, fr.FONT_HEIGHT+4+2, corDoCorpo, alpha);
			Desenhador.desenharTextoCentralizado(texto.texto, textoX+textoLargura/2, textoY+textoAltura/2, texto.cor, false);
			y += fr.FONT_HEIGHT+6;
		}
		minX = x;
		minY = y;
	...
	public void desenhar(int telaLargura, int telaAltura, Collection<TextoInformacoes> lista){
		if(fr.getStringWidth(textoDeCima)+4+larguraDosBotoes*2 > minLargura){
			minLargura = fr.getStringWidth(textoDeCima)+4+larguraDosBotoes*2;
		}
		int larguraMax = minLargura;
